...
var lstat         = rsvp.denodeify(fs.lstat);
var utimes        = rsvp.denodeify(fs.utimes);
...
JSXTranspiler.prototype.processFile = function(file) {
  return readFile(file, { encoding: this.inputEncoding || 'utf8' }).then(function(v) {
    return lstat(file).then(function(fileStats) {
      var output;
      var outputPath = path.join(this.outputPath,
                                 file.slice(this.inputPaths[0].length + 1,
                                            file.lastIndexOf(".") + 1) + "js");
      try {
        output = jsx(v, file);
      } catch (err) {
      ...
      return mkdirp(path.dirname(outputPath)).then(function() {
        return writeFile(outputPath, output, { encoding: this.inputEncoding || 'utf8' })
          .then(function() {
            return utimes(outputPath, fileStats.atime, fileStats.mtime);
          });
      });
    }.bind(this));
  }.bind(this));
...
JSXTranspiler.prototype.skipFile = function(file) {
  var outputPath = path.join(this.outputPath,
                             file.slice(this.inputPaths[0].length + 1, file.length));
  return mkdirp(path.dirname(outputPath)).then(function() {
